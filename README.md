...
第一次生成的 `review.json` 需要复核识别结果。首次委托交付 v1 ZIP，同次质检修订仍为 v1，旧尝试保存在内部 `.drafts`。向**同一个原材料目录**追加补件，再调用本 Skill，才生成 v2、v3 等新版本。待核实票正常编号和暂计已知金额，原因写在 Excel 备注；同品名发票连续排列，同开票方同日发票标注同组。个人垫付满 1000 元本身不禁报；材料类商品含税单价满 1000 元才需高值耗材登记，单张发票总价超过 1000 元需支付记录。原始文件不会移动或改名。
