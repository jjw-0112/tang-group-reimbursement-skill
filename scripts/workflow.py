...
from engine import CATEGORY_ORDER, money, money_str, natural_key, read_source, write_json
...
ATTACHMENT_LABELS = {
    ...
    "inventory": "入库单", "high_value": "高值材料审批材料", "meeting_notice": "会议通知",
}
...
def _missing_docs(record: dict[str, Any], attached: list[dict[str, Any]], public: bool) -> tuple[list[str], list[str]]:
    ...
    if category == "查收查引/专利" and "查" in description:
        ...
        if amount is not None and amount > Decimal("1000") and "payment" not in kinds:
            missing.append(ATTACHMENT_LABELS["payment"])
    if category == "材料费":
        ...
        if high_value and "high_value" not in kinds:
            missing.append("高值材料审批材料：" + "；".join(high_value))
...
def evaluate(draft: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, list[str]]]:
    ...
    for record in records:
        ...
        if printed is not None and printed >= PRIVATE_LIMIT and not item["public_transfer"]:
            ineligible.append("单张发票满 1000 元且未列为对公转账")
        if item["public_transfer"] and printed is not None and printed >= PRIVATE_LIMIT and not any(a.get("attachment_type") == "payment" for a in linked):
            pending.append("对公大额发票缺付款依据，报销金额无法确认")
        if item["category"] == "材料费" and not record.get("items_verified"):
        ...
        if item["category"] == "材料费" and record.get("items_verified"):
...
def _sort_key(record: dict[str, Any]) -> tuple[Any, ...]:
    ...
    return order, category, threshold_flag, record.get("issue_date_final") or "9999", natural_key(record["ref"])
...
def _write_workbook(path: Path, profile: dict[str, str], records: list[dict[str, Any]]) -> None:
    ...
    for cell in ws[1]:
        cell.font = Font(name="等线", size=11, bold=cell.column in {1, 3, 5})
    ...
    for category in categories:
        ...
        for record in groups[category]:
            ...
            if record["status"] == "missing":
                notes.append("缺件：" + "；".join(record["missing_docs"]))
            inventory, high_value = _item_thresholds(record) if record["category"] == "材料费" else ([], [])
            if inventory:
                notes.append("需入库：" + "；".join(inventory))
            if high_value:
                notes.append("高值手续：" + "；".join(high_value))
            if record.get("warnings"):
                notes.extend(record["warnings"])
            if record.get("manual", {}).get("notes"):
            ...
            ws.row_dimensions[row].height = min(66, max(24, 24 + len("；".join(notes)) // 75 * 14))
        ...
        for cell in ws[header_row][:6]:
            ...
            cell.font = Font(name="等线", size=11, bold=True, color=dark)
    ...
    ws.cell(total_row, 4, "含缺件票；待核实与不可报销票未计入")
    if public_subtotal:
        row = ws.max_row + 1
...
def _pdf_styles() -> tuple[ParagraphStyle, ParagraphStyle, ParagraphStyle]:
    ...
    title = ParagraphStyle("TitleCN", fontName="STSong-Light", fontSize=16, leading=23, alignment=TA_CENTER, spaceAfter=12)
    heading = ParagraphStyle("HeadingCN", fontName="STSong-Light", fontSize=11, leading=17, spaceBefore=10, spaceAfter=4)
    body = ParagraphStyle("BodyCN", fontName="STSong-Light", fontSize=9, leading=15, alignment=TA_LEFT, wordWrap="CJK")
...
def _write_pdf(path: Path, profile: dict[str, str], records: list[dict[str, Any]], attachments: list[dict[str, Any]], targets_by_ref: dict[str, list[str]], version: int) -> None:
    ...
    person_total = sum((Decimal(r["claim_amount"]) for r in records if r["status"] in {"ready", "missing"} and not r["public_transfer"]), Decimal("0"))
    public_total = sum((Decimal(r["claim_amount"]) for r in records if r["status"] in {"ready", "missing"} and r["public_transfer"]), Decimal("0"))
    ...
    summary = (
        ...
        "本包仅作离线材料核对，未进行税务平台验真；暂计金额含缺件票，不等于最终获批金额。"
    )
    ...
    section("必须补充的材料", [r for r in records if r["status"] == "missing"], "missing_docs")
    section("待核实的发票", [r for r in records if r["status"] == "pending"], "issues")
    ...
    if unmatched:
        ...
        story.append(Paragraph("无。", body))
    path.parent.mkdir(parents=True, exist_ok=True)
    doc = SimpleDocTemplate(str(path), pagesize=A4, leftMargin=40, rightMargin=40, topMargin=38, bottomMargin=38)
    doc.build(story)
    ...
    with path.open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        for record in records:
            writer.writerow({
                ...
                "关联发票ID": record["id"], "问题": "；".join(record.get("issues", [])), "SHA256": record["sha256"],
            })
...
def build(draft: dict[str, Any]) -> dict[str, Any]:
    ...
    version = int(draft.get("previous_version", 0)) + 1
    while (output_root / f"v{version}").exists() or (output_root / f".v{version}.building").exists() or (output_root / f"{_safe_name(draft['profile']['name'])}-集中报销-v{version}.zip").exists():
        version += 1
    stage = output_root / f".v{version}.building"
    stage.mkdir()
    try:
        sorted_records = sorted(records, key=_sort_key)
        formal = [r for r in sorted_records if r["status"] in {"ready", "missing"}]
        for index, record in enumerate(formal, 1):
            record["number"] = index
            amount = Decimal(record["claim_amount"])
            name = f"{index}-{_safe_name(record['description'], 34)}-{_compact_money(amount)}元{record['ext']}"
            path = _unique_path(stage / "发票", name)
            _copy_invoice(root, record, path)
            record["output_file"] = path.relative_to(stage).as_posix()
        for record in sorted_records:
            if record["status"] not in {"pending", "ineligible"}:
                continue
            subdir = "待核实" if record["status"] == "pending" else "不可报销"
            original_name = _safe_name(Path(record["ref"].split("::")[-1]).name)
            path = stage / subdir / record["id"] / original_name
            _copy_invoice(root, record, path)
            record["output_file"] = path.relative_to(stage).as_posix()
        ...
        zip_path = output_root / f"{_safe_name(profile['name'])}-集中报销-v{version}.zip"
        with ZipFile(zip_path, "w", compression=ZIP_DEFLATED, compresslevel=6) as archive:
            for path in sorted(final_dir.rglob("*")):
                if path.is_file():
                    archive.write(path, (final_dir.name + "/" + path.relative_to(final_dir).as_posix()))
        state = {
            "schema_version": 1, "version": version, "input_dir": str(root), "output_dir": str(output_root),
        ...
        return {"version": version, "zip": str(zip_path), "directory": str(final_dir), "counts": counts, "formal_count": len(formal)}
    except Exception:
